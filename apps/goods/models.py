...
class GoodsType(base_model.BaseModel):
    ...
    def __str__(self):
        return self.name
class GoodsSKU(base_model.BaseModel):
...
class IndesGoodsBanner(base_model.BaseModel):
    '''首页轮播商品展示模型类'''
    sku=models.ForeignKey('GoodsSKU',verbose_name='商品',on_delete=models.CASCADE)
...
class IndexTypeGoodsBanner(base_model.BaseModel):
...
class IndexPromotionBanner(base_model.BaseModel):
    '''首页促销活动模型类'''
